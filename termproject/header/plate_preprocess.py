...
# 번호판과 비슷한 모양의 영역들을 검출하기 위한 함수 구현
...
def preprocessing(car_no):
    ...
    kernel = np.ones((5, 17), np.uint8)                          # 닫힘 연산을 위한 마스크(커널)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)               # 명암도 영상 변환(컬러->이진)
    gray = cv2.blur(gray, (5, 5))                                # 경계가 아닌 부분 제거하기 위한 블러링
    gray = cv2.Sobel(gray, cv2.CV_8U, 1, 0, 3)                   # 소벨 에지 검출(수직 방향 에지)
    ...
    # 수직 방향 에지에 모폴로지 닫힘 연산을 가로로 긴 커널(5행 17열)을 구성하여 수행하면 가로로 긴 영역이 합쳐짐.
